...
mapcolourlabel <- 
 ...
 ggplot() +
   ...
   geom_point( data=postcode_full_County %>% arrange(CountyGap) %>% tail(5), aes(x=longitude, y=latitude), color="#78D9C5", size=2) +
    scale_size_continuous(range=c(1,20)) +
  scale_color_viridis(trans="log",alpha = 1,option = "cividis") +
    theme_void() + ylim(50,59) + coord_map() + theme(legend.position="right") 
...
#### district
Can change this to the most people seeking treatment etc later
```{r map for seeking by district colour labelled,warning=FALSE}
...
mapcolourlabel <- 
 ...
 ggplot() +
    ...
    geom_text_repel( data=postcode_full_District %>% arrange(DistrictGap) %>% tail(5), aes(x=longitude, y=latitude, label=District), size=4) +
   geom_point( data=postcode_full_District %>% arrange(DistrictGap) %>% tail(5), aes(x=longitude, y=latitude), color="#78D9C5", size=2) +
    scale_size_continuous(range=c(1,7)) +
  scale_color_viridis(trans="log",alpha = 1,option = "cividis") +
    theme_void() + ylim(50,59) + coord_map() + theme(legend.position="right") 
