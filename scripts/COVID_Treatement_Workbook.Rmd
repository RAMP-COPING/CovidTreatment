...
dupids <- DupIDdat$ID[(DupIDdat$`June 2020` > 1 | AnySeekSums$`July 2020` > 1 |
                         DupIDdat$`August 2020` > 1 | AnySeekSums$`September 2020` > 1 |
                         DupIDdat$`October 2020` > 1 | AnySeekSums$`November 2020` > 1 |
                         DupIDdat$`December 2020` > 1 | AnySeekSums$`January 2021` > 1)]
...
NEED TO GO BACK AND GET THESE VARIABLES. CHECK ALICIA AND LAURA SCRIPT FOR NAMING ETC
```{r dummy type of barriers}

treat_dat <- treat_dat %>%
  mutate(TypeReceived  = case_when( == 1 ~ "Systemic",
                                == 1 ~ "Systemic",
                                 == 1 ~ "Systemic",
                                 == 1 ~ "Systemic",
                                 == 1 ~ "Client-level",
...
### Number of times sought treatment (relative to number of times I responded)
