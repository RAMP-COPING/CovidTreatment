...
cols.nbr <- c("ID")
keepcols.ramp <- c("externalDataReference","dem2.how_old_are_you","dem2.which_gender_do_you_identify_with","dem2.where_in_the_uk_do_you_live.txt")
...
demgen <- subset(dem,select = c("ID","Gender"))
```
...
treat_dat <- merge(treat_dat,demgen,keep="all")
```
...
# postcode_full_barrier <- left_join(postcode_barrier,postcode_df_area,by="Outcode")
# 
# # drop rows with no ID data as this indicates they did not exist in the downloaded Uk reference data
# postcode_full_seek <- postcode_full_seek[(!is.na(postcode_full_seek$id) == T),]
# postcode_full_barrier <- postcode_full_barrier[(!is.na(postcode_full_barrier$id) == T),]
...
check treatment seeking
```{r check head and count looks fine seek}
# head(area_count_seek)
# sum(area_count_seek$nCounty)
```

check treatment barriers
```{r check head and count looks fine barrier}
# head(area_count_barrier)
# sum(area_count_barrier$nCounty)
```
