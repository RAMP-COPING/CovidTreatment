...
demCOPE.glad.AGEetc <- readRDS(file = paste0(data_path, "/glad/dem_glad.RDS"))
...
GLADdemvars <- demCOPE.glad.AGEetc %>%
  select("dem.which_gender_do_you_identify_with","dem.questions_based_ethnic_origin","dem.how_old_are_you_now.txt") %>%
  rename(Gender=dem.which_gender_do_you_identify_with,
...
# Demographics cleaning and summaries
...
## join to treatment data and summarise
...
```{r map for gap by county colour no label,warning=FALSE}
...
```{r map for gap by district colour no label,warning=FALSE}
...
#### county
```{r map for gap by county colour labelled,warning=FALSE}
bb <- c(10,50,100,300,500)
...
```{r map for gap by district colour labelled markers,warning=FALSE}
...
```{r map for gap by district colour markers no labels,warning=FALSE}
...
```{r map for gap by district colour labelled top ten only,warning=FALSE}
