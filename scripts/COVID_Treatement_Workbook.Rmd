...
treat_dat <- treat_dat %>%
  mutate(ReasonSeek = case_when(reasonhelp_new_mh == 1 | reasonhelp_new_med == 1 ~ "Newly emerging symptoms",
...
By sample
```{r check barriers by wave supported by sample}
treat_dat %>%
  group_by(wave,Sample) %>%
  freq(PreventHelp_Supported)
```
...
By sample
```{r check barriers by wave client led by sample}
treat_dat %>%
  group_by(wave,Sample) %>%
  freq(PreventHelp_SelfGuided)
```
