...
cols.nbr <- c("ID")
keepcols.ramp <- c("Login ID","dem2.how_old_are_you","dem2.which_gender_do_you_identify_with","dem2.where_in_the_uk_do_you_live.txt")
cols.ramp <- c("ID","Age","Gender","Postcode")
...
dem <- rbind(dem.cope.glad,dem.cope.edgi)
dem <- left_join(dem,dem.cope.nbr)
dem <- merge(dem,dem.ramp,all=TRUE)
...
Receiving ANY instances of this type of treatment (i.e., if they sought and received supported help from 3 sources ina  wave, it will just register once)
...
### Frequency of systemic / client barriers by wave and treatment type
...
```
### Frequency of different levels of helpfulness by treatment type (cupported/client led) by wave
...
check county data
```{r check head and count looks fine county}
head(postcode_full_County)

```

check district data
```{r check head and count looks fine district}
...
```{r map for seeking by county colour no label,warning=FALSE}
...
mapcolour <- 
 ...
 ggplot() +
    ...
    geom_point( aes(x=longitude, y=latitude, size=SeekCountyTotal, color=CountyGap), alpha=0.8) +
    scale_size_continuous(range=c(1,20)) +
  scale_color_viridis(trans="log",alpha = 1,option = "D") +
    theme_void() + ylim(50,59) + coord_map() + theme(legend.position="right") 
...
```{r map for seeking by district colour no label,warning=FALSE}

mapcolour <- 
 postcode_full_District %>%
  
 ggplot() +
    geom_polygon(data = UK, aes(x=long, y = lat, group = group), fill="grey", alpha=1) +
    geom_point( aes(x=longitude, y=latitude, size=SeekDistrictTotal, color=DistrictGap), alpha=0.5) +
...
### Labelled the 5 counties with the greatest treatment gap

#### county
Can change this to the most people seeking treatment etc later
```{r map for seeking by county colour labelled,warning=FALSE}
...
mapcolourlabel <- 
 ...
 ggplot() +
    geom_polygon(data = UK, aes(x=long, y = lat, group = group), fill="grey", alpha=0.5) +
    geom_point( aes(x=longitude, y=latitude, size=SeekCountyTotal, color=CountyGap), alpha=0.8) +
    geom_text_repel( data=postcode_full_County %>% arrange(CountyGap) %>% tail(5), aes(x=longitude, y=latitude, label=County), size=4) +
   geom_point( data=postcode_full_County %>% arrange(CountyGap) %>% tail(5), aes(x=longitude, y=latitude), color="#78D9C5", size=2) +
    scale_size_continuous(range=c(1,20)) +
  scale_color_viridis(trans="log",alpha = 1,option = "D") +
    theme_void() + ylim(50,59) + coord_map() + theme(legend.position="right") 
...
#### district labelled with marker points
orange markers for top 5 biggest gap, with labels

```{r map for seeking by district colour labelled markers,warning=FALSE}
...
mapcolourlabel <- 
 ...
 ggplot() +
    geom_polygon(data = UK, aes(x=long, y = lat, group = group), fill="grey", alpha=0.5) +
    geom_point( aes(x=longitude, y=latitude, size=SeekDistrictTotal, color=DistrictGap), alpha=0.8) +
    geom_text_repel( data=postcode_full_District %>% arrange(DistrictGap) %>% tail(5), aes(x=longitude, y=latitude, label=District), size=5, face = "bold",colour = "black") +
   geom_point( data=postcode_full_District %>% arrange(DistrictGap) %>% tail(5), aes(x=longitude, y=latitude), color="orange", size=2) +
    scale_size_continuous(range=c(1,7)) +
  scale_color_viridis(trans="log",alpha = 1,option = "D") +
    theme_void() + ylim(50,59) + coord_map() + theme(legend.position="right") 
...
```{r map for seeking by district colour markers no labels,warning=FALSE}
...
#### district - only top ten gap 
Can change this to the most people seeking treatment etc later
```{r map for seeking by district colour labelled top ten only,warning=FALSE}
...
mapcolourlabel <- 
 ...
 ggplot() +
    geom_polygon(data = UK, aes(x=long, y = lat, group = group), fill="grey", alpha=0.5) +
    geom_point( aes(x=longitude, y=latitude, size=SeekDistrictTotal, color=DistrictGap), alpha=0.8) +
    geom_text_repel( data=postcode_full_District %>% arrange(DistrictGap) %>% tail(10), aes(x=longitude, y=latitude, label=District), size=4, face = "bold",colour = "black") +
...
#### district - only top five gap 
Can change this to the most people seeking treatment etc later
```{r map for seeking by district colour labelled top five only,warning=FALSE}
...
mapcolourlabel <- 
 ...
 ggplot() +
    geom_polygon(data = UK, aes(x=long, y = lat, group = group), fill="grey", alpha=0.5) +
    geom_point( aes(x=longitude, y=latitude, size=SeekDistrictTotal, color=DistrictGap), alpha=0.8) +
    geom_text_repel( data=postcode_full_District %>% arrange(DistrictGap) %>% tail(5), aes(x=longitude, y=latitude, label=District), size=4, face = "bold",colour = "black") +
