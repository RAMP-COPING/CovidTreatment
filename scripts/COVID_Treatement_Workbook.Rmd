...
treat_dat <- treat_dat %>%
  mutate(TypeReceived  = case_when(wherehelp_received_emergency_mh == 1 ~ "Supported",
                               wherehelp_received_existing_mh_team == 1 ~ "Supported",
                                wherehelp_received_online_talk_therapy == 1 ~ "Supported",
                                wherehelp_received_gp == 1 ~ "Supported",
                                wherehelp_received_nonnhs_phone == 1 ~ "Supported",
                                wherehelp_received_nhs_111 == 1 ~ "Supported",
                                wherehelp_received_gov_website == 1 ~ "Self-guided",
                                wherehelp_received_selfguide_online == 1 ~ "Self-guided",
                                wherehelp_received_nongov_website == 1 ~ "Self-guided",
                                wherehelp_received_struc_therapy == 1 ~ "Self-guided")) %>%
  mutate(AnyReceived = case_when(TypeReceived  == "Supported" |
                                   TypeReceived  == "Self-guided" ~ 1,
                                 TRUE ~ 0))
...
##### received treatment at any time (How many times, how many total response)

```{r treatment receipt in any wave}


```

##### Received help having sought it (include proprtion of times help is received having sought it)
Check if anyone has not received help at all having sought it, check proportion of times help received having sought it, separated into supported vs self-guided treatments
...
##### total barriers that are client focused or systemic by wave

Create dummy variables indicating systemic and client barriers in a column each
```{r dummy variables for overall rates of  client or systemic barriers}

systemic <- rowSums(sapply(treat_dat[grep("prevent",names(treat_dat))], function(v) v %in% c("Systemic")))
treat_dat$BarrierToTreatment.Systemic <- ifelse(systemic>0,1,0)

client <- rowSums(sapply(treat_dat[grep("prevent",names(treat_dat))], function(v) v %in% c("Client-level")))
treat_dat$BarrierToTreatment.Client <- ifelse(client>0,1,0)

other <- rowSums(sapply(treat_dat[grep("prevent",names(treat_dat))], function(v) v %in% c("Other")))
treat_dat$BarrierToTreatment.Other <- ifelse(other>0,1,0)
```

##### Count number of systemic vs client led barriers over whole sample for service type
...
### Frequency of treatment receipt by wave

```{r treatment receipt by wave}
treat_dat %>%
  group_by(wave) %>%
  freq(TypeReceived)
...
Systemic
```{r check barriers by wave systemic}
treat_dat %>%
  group_by(wave) %>%
  freq(BarrierToTreatment.Systemic)
```
Client
```{r check barriers by wave client}

treat_dat %>%
  group_by(wave) %>%
  freq(BarrierToTreatment.Client)

```

Other
```{r check barriers by wave other}
treat_dat %>%
  group_by(wave) %>%
  freq(BarrierToTreatment.Other)
```
...
### Frequency of type of treatment sought

```{r freq treatment type}
